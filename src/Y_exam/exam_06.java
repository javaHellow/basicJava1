...
//		[6-1] 다음과 같은 멤버변수를 갖는 클래스를 정의하시오 SutdaCard 
//		타 입 변수명 설 명
...
//		[6-3] 다음과 같은 멤버변수를 갖는Student 클래스를 정의하시오  .
//		타 입         변수명	 설 명
//		String  name 	학생이름
//		int 	ban 	반
//		int 	no 		번호
...
//		[6-7] 문제6-6 에서 작성한 클래스메서드   getDistance()를 MyPoint클래스의 인스턴스메서 
//		드로 정의하시오.
...
//		[6-9] 다음은 컴퓨터 게임의 병사(marine) 를 클래스로 정의한 것이다 이 클래스의 멤버 static  .
...
//	}	
//		
//		weapon, armor은 모든 Marine인스턴스에 대해 동일한 값이어야 하므로.
		
//		[6-10] 다음 중 생성자에 대한 설명으로 옳지 않은 것은 모두 고르시오 ? (b,e )
//				a. 모든 생성자의 이름은 클래스의 이름과 동일해야한다	
//				b. 생성자는 객체를 생성하기 위한 것이다	      			객체를 생성하는 것은 new 이다
//				c. 클래스에는 생성자가 반드시 하나 이상 있어야 한다
//				d. 생성자가 없는 클래스는 컴파일러가 기본 생성자를 추가한다
//				e. 생성자는 오버로딩 할 수 없다. 						오버로딩이 가능하며 하나의 클래스에 여러개의 생성자를 정의할수 있다.
	
		
//		[6-11] 다음 중 this에 대한 설명으로 맞지 않은 것은? (모두 고르시오 )
//				a. 객체 자신을 가리키는 참조변수이다		b 클래스의  클래스 변수, 클래스 메서드에는 사용불가능
...
//				d. 매개변수의 이름이 달라야 한다.
//		
//		리턴타입은 상관없고 메서드의 이름이 같거나 매개변수의 개수 또는 타입이 달라야한다.
...
//				c. 초기화 블럭보다 생성자가 먼저 수행된다		명시적 -> 초기화 블럭{} -> 생성자
//				d. 명시적 초기화를 제일 우선적으로 고려해야 한다
//				e. 클래스변수보다 인스턴스변수가 먼저 초기화 된다 	클래스 변수가 먼저 초기화 됨
...
//				a. 자동 초기화되므로 별도의 초기화가 필요없다  -자동초기화가 안되고 직접 맞는 값으로 초기하 해줘야함				
//				b. 지역변수가 선언된 메서드가 종료되면 지역변수도 함께 소멸된다
//				c. 매서드의 매개변수로 선언된 변수도 지역변수이다
//				d. 클래스변수나 인스턴스변수보다 메모리 부담이 적다
//				e. 힙(heap)영역에 생성되며 가비지 컬렉터에 의해 소멸된다
//				힙에는 인스턴스변수 또는 ()가 생성되며 지역변수는 콜스택에 생성됨
...
//				b. println메서드를 제외한 나머지 메서드들은 모두 종료된 상태이다  대기상태이다
...
//			static int cv = 20;
//			int iv2 = cv;
//			static int cv2 = iv;			 // A  객체를 생성안해줘서
//			static void staticMethod1() {
//			System.out.println(cv);
//			System.out.println(iv);			 // B 라인  메서드변수사용 불가
//			}
...
//			staticMethod1();
//			instanceMethod1(); 				// D 라인			인스턴스 메서드 사용불가
//			}
...
//				str += "456";
//	}
//		public static void main(String[] args)	"ABC123456"
//		{
//			String str = "ABC123";
